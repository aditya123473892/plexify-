...
const PPFManagement = () => {
  const [ppfDetails, setPpfDetails] = useState([
    { name: '', phone: '', email: '', ppfAccountNumber: '', contribution: '', nominee: '' },
  ]);
  ...
  const addPpfDetail = () => {
    setPpfDetails([...ppfDetails, { name: '', phone: '', email: '', ppfAccountNumber: '', contribution: '', nominee: '' }]);
  };
      ...
      {ppfDetails.map((ppf, index) => (
        <FieldSection title={`PPF Detail ${index + 1}`} key={index}>
          ...
          <InputWithIcon
            ...
            onChange={(e) => handlePpfChange(index, 'ppfAccountNumber', e.target.value)}
          />
          <InputWithIcon
