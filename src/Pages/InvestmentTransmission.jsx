...
const InvestmentTransmission = () => {
  const [transmissions, setTransmissions] = useState([
    { type: "", accountNumber: "", date: "", beneficiary: "" },
  ]);
    ...
    <div className="min-h-screen shadow-2xl bg-white p-6 rounded-lg md:mt-10 mt-20">
      <header className="mb-8">
        <h1 className="text-3xl font-bold text-gray-800 mb-2">
          Manage Investment Transmission
        </h1>
        <p className="text-gray-600">
          Keep track of your investments that need to be transmitted to
          beneficiaries.
        </p>
      </header>
      ...
      {transmissions.map((transmission, index) => (
        <FieldSection title="Investments to Transmit">
          <InputWithIcon
            ...
            onChange={(e) =>
              handleTransmissionChange(index, "type", e.target.value)
            }
          />
          <InputWithIcon
            icon={<FaTrash className="text-[#538d2dfd] mx-2" />}
            ...
            onChange={(e) =>
              handleTransmissionChange(index, "accountNumber", e.target.value)
            }
          />
          <InputWithIcon
            icon={<FaTrash className="text-[#538d2dfd] mx-2" />}
            ...
            onChange={(e) =>
              handleTransmissionChange(index, "date", e.target.value)
            }
          />
          <InputWithIcon
            icon={<FaTrash className="text-[#538d2dfd] mx-2" />}
            ...
            onChange={(e) =>
              handleTransmissionChange(index, "beneficiary", e.target.value)
            }
          />
          <button
